...
public class DungeonGame {
	...
	public int calculateMinimumHP(int[][] dungeon) {
		...
		for (int i = row - 2; i >= 0; i--) {
			for (int j = column - 2; j >= 0; j--) {
				tem[i][j] = Math.min(c(dungeon[i][j], tem[i + 1][j]), c(dungeon[i][j], tem[i][j + 1]));
			}
		}
	...
	public static void main(String[] args) {
		...
		int minimumHP = game.calculateMinimumHP(dungeon);

		System.out.println("Minimum initial health required: " + minimumHP);
	}
}
